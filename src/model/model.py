...
from preprocess import get_data, stats_columns, ven_map
...
class Model(ABC):
    ...
    def _get_vhats(self, X, y, projected_dim):
        ...
        most_important_indices = np.argpartition(np.linalg.eig(Q)[0], -projected_dim)[
            -projected_dim:
        ]
    ...
    def predict(self, team1: str, team2: str, venue: str) -> np.ndarray:
        ...
        if venue not in ven_map.keys():
            raise ValueError(
                "venue must be one of the following strings: 'H', 'N', and 'A'."
            )
        ...
        for stat in stats_columns:
            x.append(
                team_rolling_avg[f"Team_{stat}_avg"]
                - opp_rolling_avg[f"Team_{stat}_avg"]
            )
        x = np.array(x)
        x = np.reshape(x, (-1, 1))
        # project x onto vhat
    ...
    @staticmethod
    def _sigmoid_deriv(z: float) -> float:
        s = Model._sigmoid(z)
        return s * (1 - s)
...
class LogisticRegression(Model):
    ...
    def train(self, epochs=10000) -> None:
        ...
        self._w = np.zeros((d + 1, 1))

        for _ in range(epochs):
            yhats = self._forward(phi)
            # print('yh',yhats.shape)
            u = self._bce_grad(self._y, yhats, phi, self.w)
            self._w = self._w - self._lr * u
    ...
    def _forward(self, phi):
        # print(phi)
        # print(self._w)
        q1 = self._sigmoid(phi.dot(self._w))
        # print(q1)
        return q1
...
class MLP(Model):
    ...
    def __init__(self, year: int, n_hidden=1, width_hidden=8, lr=0.00001, dropout=0.1):
        ...
        self.W = [
            np.random.normal(
                0,
                1 / (self._alphas.shape[1]) ** 2,
                (self._alphas.shape[1], width_hidden),
            )
        ]
        ...
        for i in range(n_hidden):
            self.W.append(
                np.random.normal(0, 1 / (n_hidden) ** 2, (width_hidden, width_hidden))
            )
    ...
    @staticmethod
    def _bce_grad_simplified(y, yhats):
    ...
    def _forward(self, phi: np.ndarray, train=True):
        ...
        for i in range(len(self.W) - 1):
            ...
            if train:
                ...
                a /= (
                    1 - self.dropout
                )  # scale during eval to maintain correct expected val
    ...
    def backward(self, y, yhats, lambda_=1):
        ...
        out_err = self._bce_grad_simplified(y, yhats)  # dL/df
        ...
        for i in reversed(range(len(self.W) - 1)):
            ...
            delta = np.dot(delta, self.W[i + 1].T) * self._relu_deriv(
                self.zs[i - 1]
            )  # backprop through relu
            # dL/dw(l) = a(l-1) * delta(l)
            W_grads[i] = (
                np.dot(self.activations[i].T, delta) + lambda_ * 2 * self.W[i]
            )  # L2 norm to reduce overfitting
            # dL/db(1) = delta(l)
            b_grads[i] = np.sum(delta, axis=0, keepdims=True)

            # layer norm grad
            delta = self._layer_norm_backward(
                delta, self.zs[i - 1]
    ...
    def _layer_norm_backward(self, grad_output, z):
        epsilon = 1e-8
        n, features = z.shape
        ...
        grad_variance = np.sum(
            grad_output * (z - mean) * (-0.5) * (std**-3), axis=1, keepdims=True
        )
        grad_mean = np.sum(
            grad_output * (-1 / std), axis=1, keepdims=True
        ) + grad_variance * np.mean(-2 * (z - mean), axis=1, keepdims=True)

        return (
            grad_z_norm
            + (grad_variance * 2 * (z - mean) / features)
            + (grad_mean / features)
        )
